...
def earliest_ancestor(ancestors, starting_node):
    ...
    while queue.size() > 0:
        ...
        # if the current is bigger than the curr longest path, then make the longest path the current
        if len(current_path) > len(longest_path):
            ...
            if current_path[-1] < longest_path[-1]:
                longest_path = current_path
        print(current_path[-1],current_path, 'curr path')
